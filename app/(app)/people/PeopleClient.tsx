...
import { Avatar, useApp } from '@/components/AppShell'
...
export default function PeopleClient({ initialUsers }: { initialUsers: User[] }) {
  ...
  useEffect(() => { if (filter !== 'active') load() }, [filter])

  async function restore(userId: string) {
    await fetch(`/api/users/${userId}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ archived: false }) })
    load()
  }

  async function resetPassword(userId: string) {
    const res = await fetch(`/api/users/${userId}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'reset-password' }) })
  ...
  return (
    <div>
      <div style={{ display: 'flex', gap: 9, marginBottom: 20, flexWrap: 'wrap', alignItems: 'center' }}>
        <select value={filter} onChange={e => { setFilter(e.target.value as 'active' | 'archived' | 'all') }} style={selStyle}>
      ...
      {loading ? <p style={{ color: 'var(--muted)' }}>Loading…</p> : (
        users.map(u => (
          <div key={u.id} style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 11, padding: '13px 15px', marginBottom: 7, display: 'grid', gridTemplateColumns: 'auto 1fr auto auto', gap: 13, alignItems: 'center', opacity: u.archived ? 0.55 : 1 }}>
            <Avatar name={u.displayName} size={36} />
            <div>
              <b style={{ fontWeight: 600 }}>{u.displayName}</b>
              <div style={{ fontSize: 13, color: 'var(--muted)', marginTop: 2 }}>
                {u.jobTitle}{u.companies.length > 0 && <span> · {u.companies.map(c => c.name).join(', ')}</span>}
              </div>
              <div style={{ display: 'flex', gap: 5, marginTop: 4, flexWrap: 'wrap' }}>
                {u.companies.map(c => <span key={c.id} style={{ width: 7, height: 7, borderRadius: '50%', background: c.color, display: 'inline-block' }} />)}
              </div>
            </div>
            <span style={{ fontSize: 11.5, fontWeight: 600, padding: '3px 9px', borderRadius: 20, whiteSpace: 'nowrap', background: u.archived ? '#F3F4F5' : u.role === 'CEO' ? '#EEF0FE' : '#EFF2F5', color: u.archived ? '#9AA3AD' : u.role === 'CEO' ? '#3B4DBF' : 'var(--muted)' }}>
      ...
      {tempPw && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(18,24,31,.45)', zIndex: 60, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
          <div style={{ background: 'var(--surface)', borderRadius: 14, padding: '28px 28px', maxWidth: 400, width: '100%', boxShadow: '0 18px 48px rgba(18,24,31,.22)' }}>
            ...
            <button onClick={() => setTempPw(null)} style={{ marginTop: 18, width: '100%', background: 'var(--ink)', color: '#fff', border: 0, borderRadius: 9, padding: 11, fontSize: 14, fontWeight: 600, cursor: 'pointer', minHeight: 44 }}>Done — I copied it</button>
          </div>
        </div>
      )}
...
function AddPersonModal({ onClose, onCreated }: { onClose: () => void; onCreated: (tempPw: string) => void }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true)
    try {
      const res = await fetch('/api/users', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name, displayName: displayName || name, email, jobTitle, functionGroup, role, companyIds }) })
      const data = await res.json()
          ...
          <div style={{ padding: '15px 18px', display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
              <label style={{ position: 'relative', cursor: 'pointer', flexShrink: 0, display: 'block' }}>
                <Avatar name={user.displayName} size={64} photoUrl={previewUrl} />
                <span style={{ position: 'absolute', inset: 0, borderRadius: '50%', background: 'rgba(0,0,0,.38)', display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: 0, transition: 'opacity .15s' }}
              ...
              <div>
                <div style={{ fontSize: 13.5, fontWeight: 600, color: 'var(--ink)' }}>{uploading ? 'Uploading…' : 'Profile photo'}</div>
                <div style={{ fontSize: 12, color: 'var(--ink-3)', marginTop: 2 }}>Click the photo to upload a new image</div>
              </div>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 11 }}>
