...
export default function TeamClient({ initialData }: { initialData: TeamData }) {
  ...
  const { groups, summary } = initialData
  const orderedGroups = GROUP_ORDER.filter(g => groups[g]?.length).map(g => [g, groups[g]] as [string, TeamUser[]])
  const extraGroups = Object.entries(groups).filter(([g]) => !GROUP_ORDER.includes(g))

  return (
    <div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(168px,1fr))', gap: 10, marginBottom: 22 }}>
        <SummaryCard value={summary.doneToday} label="tasks done today" />
        <SummaryCard value={summary.peopleBehind} label="people behind" warn={summary.peopleBehind > 0} />
        <SummaryCard value={summary.stuckOver48h} label="stuck over 48h" warn={summary.stuckOver48h > 0} />
        <SummaryCard value={summary.unassigned} label="tasks unassigned" />
      </div>
...
function PersonRow({ user, isOpen, onToggle }: { user: TeamUser; isOpen: boolean; onToggle: () => void }) {
  ...
  return (
    <article style={{ background: 'var(--surface)', border: `1px solid var(--line)`, borderLeft: user.isBehind ? '3px solid var(--working)' : '1px solid var(--line)', borderRadius: 11, marginBottom: 7, overflow: 'hidden' }}>
      <button onClick={onToggle} style={{ width: '100%', background: 'none', border: 0, textAlign: 'left', padding: '13px 14px', display: 'grid', gridTemplateColumns: '1fr auto auto', gap: 13, alignItems: 'center', cursor: 'pointer' }}>
        <span>
          <span style={{ fontSize: 15.5, fontWeight: 600, letterSpacing: '-0.01em', display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 4 }}>
            ...
            <span style={{ display: 'inline-flex', gap: 3, marginLeft: 3, verticalAlign: '1px' }}>
              {user.companies.map(c => <span key={c.id} style={{ width: 7, height: 7, borderRadius: '50%', background: c.color, display: 'inline-block' }} />)}
            </span>
          </span>
        ...
        <span style={{ position: 'relative', width: 34, height: 34, flexShrink: 0, display: 'block' }}>
          <svg width={34} height={34} style={{ transform: 'rotate(-90deg)', display: 'block' }}>
            ...
            <circle cx={17} cy={17} r={14} fill="none" stroke={user.isBehind ? 'var(--working)' : mainColor} strokeWidth={4} strokeLinecap="round" strokeDasharray={circumference} strokeDashoffset={dashOffset} style={{ transition: 'stroke-dashoffset .3s' }} />
          </svg>
          <span style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10.5, fontWeight: 700 }}>{pct}%</span>
        </span>
