...
interface RecurringTemplate { id: string; title: string; companyId: string; assigneeId: string | null; frequency: string; weekdays: number[]; active: boolean }
...
export default function SettingsClient({ initialSettings, initialRecurring, initialHandoffs, initialChecklists, initialUsers }: {
  ...
  const [recurring, setRecurring] = useState<RecurringTemplate[]>(initialRecurring)
  ...
  async function toggleRecurring(id: string, active: boolean) {
    await fetch('/api/recurring', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id, active }) })
    setRecurring(prev => prev.map(r => r.id === id ? { ...r, active } : r))
  }

  async function toggleHandoff(id: string, active: boolean) {
    await fetch('/api/handoff-rules', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id, active }) })
  ...
  async function addCompany(e: React.FormEvent) {
    e.preventDefault()
    setSaving(true)
    await fetch('/api/companies', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: newCoName, color: newCoColor }) })
        ...
        <div style={{ paddingTop: 12 }}>
          {!addCompanyOpen ? (
            ...
            <form onSubmit={addCompany} style={{ display: 'flex', gap: 9, alignItems: 'flex-end', flexWrap: 'wrap' }}>
              ...
              <button type="button" onClick={() => setAddCompanyOpen(false)} style={{ background: 'none', border: '1px solid var(--line)', borderRadius: 9, padding: '9px 14px', fontSize: 14, cursor: 'pointer', minHeight: 44 }}>Cancel</button>
            </form>
          )}
        </div>
      </Panel>
...
function Rule({ label, sub, value, onToggle }: { label: string; sub: string; value: boolean; onToggle: (v: boolean) => void }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 13, padding: '11px 0', borderBottom: '1px solid var(--line-soft)', fontSize: 14 }}>
      <div>{label}<div style={{ fontSize: 12.5, color: 'var(--muted)', marginTop: 2 }}>{sub}</div></div>
